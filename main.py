...
SECRET_KEY = os.getenv('SECRET_KEY', '17dfb1a5c43144b0f041be2c14289aab236de064db1d59d945abfb206dba0d08')
ALGORITHM = "HS256"
...
@app.get("/api/items")
def get_items(current_user: Annotated[User, Depends(get_current_active_user)], limit: Union[int, None] = 12):
    query = {
        "size": limit,
        "query": {
            "match_all": {}
...
def read_item(current_user: Annotated[User, Depends(get_current_active_user)], item_id: str, q: Union[str, None] = None):
    query = {
        "size": 1,
        "query": {
            "match": {"id": item_id}
...
@app.post("/api/search")
def search(current_user: Annotated[User, Depends(get_current_active_user)], file: UploadFile = File(...)):
    try:
        contents = file.file.read()
        if not os.path.exists('public/uploads/' + current_user.get('username')):
            os.mkdir('public/uploads/' + current_user.get('username'))

        upload_path = 'public/uploads/' + current_user.get('username') + '/' + file.filename
...
def analyze(current_user: Annotated[User, Depends(get_current_active_user)], file: UploadFile = File(...)):
    try:
        if not os.path.exists('public/uploads/' + current_user.get('username')):
            os.mkdir('public/uploads/' + current_user.get('username'))
        contents = file.file.read()
...
@app.post("/api/add")
async def create_item(current_user: Annotated[User, Depends(get_current_active_user)], item: Item):
    embedding_objs = DeepFace.represent(img_path=item.img_path, model_name=MODEL_NAME, detector_backend='retinaface', align=True)
